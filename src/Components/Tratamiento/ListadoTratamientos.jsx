...
import TablaTratamientos from "./TablaTratamientos.jsx";
...
function ListadoTratamientos() {
  ...
  const [buscarTratamiento, setBuscarTratamiento] = useState("");
  const [tratamientos, setTratamientos] = useState([]);
  const [tratamientosFiltrados, setTratamientosFiltrados] = useState([]);
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 10;

  const consultarTratamientos = useCallback(async () => {
    try {
      Swal.fire({
        title: "Consultando...",
      ...
      if (response.status === 200) {
        ...
        setTratamientosFiltrados(response.data);
        setCurrentPage(1);
      }
    } catch (error) {
      Swal.close();
  ...
  const currentItems = tratamientosFiltrados.slice(
    indexOfFirstItem,
    indexOfLastItem
  );

  const totalPages = Math.ceil(tratamientosFiltrados.length / itemsPerPage);
  ...
  const agregarTratamiento = () => {
    window.location.href = "/formularioTratamiento";
  };
  ...
  return (
    <div className="p-6 relative">
      ...
      <TablaTratamientos
        tratamientos={currentItems}
        actualizarTratamientos={consultarTratamientos}
      />
